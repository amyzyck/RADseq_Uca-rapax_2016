...
# Population analyses on Neutral dataset  
...
#### Make genind object
```{r}
my_vcf <- read.vcfR("neutralsnploci.recode.vcf")
...
### Repeat for 1 random SNP per contig
```{r}
my_vcf_u <- read.vcfR("neutralsnploci.filtered1SNPper.vcf")
...
```{r}
dirs = c("/home/azyck/Fiddler_Crab/ddocent_env/NeutralSNP/neutraldata-filt-D200-chain1", "/home/azyck/Fiddler_Crab/ddocent_env/NeutralSNP/neutraldata-filt-D300-chain1")
```

```{r}
eems.plots(mcmcpath = dirs, plotpath = "/home/azyck/Fiddler_Crab/ddocent_env/NeutralSNP/neutraldata-filt-maf05-All-plots",
...
neutral <- ggplot(data = meltedN, aes(Var2, Var1, fill = value))+ geom_tile(color = "white")+ 
  ...
  ggtitle(expression(atop("Pairwise FST, WC (1984) Neutral", atop(italic("N = 320, L = 8,117"), ""))))+
...
```{r}
write.table(gindF.fst.mat.triR, file="neutralloci-Reg.pwfst",row.names = T,col.names = T)
...
```{r}
#Plot DAPC plot 
col12 <- c("darkorange2","orange1","chocolate1","darkorange","green3","green4","gold","yellow","skyblue","skyblue3","slateblue1","steelblue4")

scatter(dapc1, scree.da=FALSE, col = col12, bg="white", pch=20, cell=0, cstar=0, solid=.7,
cex=3,clab=0, leg=TRUE, txt.leg= pop_order, xlab = "PC1: 17.91% Variance Explained", ylab = "PC2: 14.75% Variance Explained")
```

#### Calculating contribution of PCs 1 and 2
...
```{r}
setPop(stratted.filt) <- ~Population

dapc1 <- dapc(stratted.filt, pop(stratted.filt))
